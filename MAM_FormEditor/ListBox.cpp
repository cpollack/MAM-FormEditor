...
CListBox::CListBox(System::String^ name, int x, int y) {
	init();
	Name = name;
	Caption = "";
	X = x;
...
CListBox::CListBox(rapidjson::Value* vWidget) : CWidget(vWidget) {
	init();
...
void CListBox::Save(rapidjson::Document* document, rapidjson::Value* vWidget) {
...
void CListBox::CreateListBoxImage() {
	if (pnl) delete pnl;
...
void CListBox::CreateCaptionTexture() {
	if (cpt) {
		delete cpt;
		cpt = nullptr;
	}
...
void CListBox::Draw(Graphics^ gr, Point pos) {
...
Point CListBox::MouseDrag(Point dragPos, Point wPos, Point dragOffset, int dragMode) {
	int tWidth = Width;
	int tHeight = Height;

	Point rPoint = CWidget::MouseDrag(dragPos, wPos, dragOffset, dragMode);

	if (tWidth != Width || tHeight != Height) CreateListBoxImage();
